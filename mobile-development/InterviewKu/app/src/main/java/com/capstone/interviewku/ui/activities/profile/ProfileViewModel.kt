...
class ProfileViewModel @Inject constructor(
    ...
    fun getUserIdentity() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.success) {
                    _userIdentity.value = Result.Success(response.data)
                } else {
                    val error = Exception("Failed to fetch user info")
                    Log.e("ProfileViewModel", "Error fetching user info. Response: $response", error)
                    _userIdentity.value = Result.Error(SingleEvent(error))
                }
            } catch (e: Exception) {
    ...
    fun editUserIdentity(
        ...
        try {
            userRepository.editUserIdentity(firstName, lastName, jobPositionId, gender, dateBirth, currentCity)
